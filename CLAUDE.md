...
- **Dual-bar (2026-05-07 spec)**: bar branches on whether *effective* actual data exists.
  ...
  - **Effective dates** via `effectiveActualStart(t)` / `effectiveActualEnd(t)`:
    ...
    - **End (2026-05-26; generalized 2026-05-27)**: branching order matters. (1) If status matches Closed/Done/Complete → return `t.actualEnd || t.resolvedAt || ''` (closed tasks own their dates). (2) Else if `t.end` exists and `todayStr > t.end` → return today (catches **any** non-closed past-plan task: Blocked / Late Start / To Do / In Progress whose `t.actualEnd` is stale or matches `t.end`). (3) Else if status === 'in progress' → return today. (4) Else return `t.actualEnd || ''`. The 2026-05-27 generalization (branch 2) replaced an earlier In-Progress-only check that left Blocked / Late Start tasks with their Late hatch cut short of the today line.
